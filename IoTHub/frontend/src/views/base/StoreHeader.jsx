...
function StoreHeader() {
    ...
    return (
        <div>
            <nav className="navbar navbar-expand-lg navbar-white navbar-custom">
                <div className="container">
                    <Link className="navbar-brand text-white" to="/" > <img src={'/IotHub.png'} alt="Logo" width="150" height="70" className="d-inline-block align-top" />
                    </Link>
                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            ...
                            <li className="nav-item dropdown text-white">
                                <a className="nav-link dropdown-toggle text-white navbar-custom" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" >
                                    About Us
                                </a>
                                <ul className="dropdown-menu " aria-labelledby="navbarDropdown">
                                    <li><Link to={'/about'} className="dropdown-item"><i className='fas fa-user'></i> What is IotHub ?</Link></li>
                                    <li><Link to={'/contactus'} className="dropdown-item"><i className='fas fa-user'></i> Contact us</Link></li>
                                </ul>
                            </li>
                            <Link className="navbar-brand text-white text-cust" to="/products/"  >Products</Link>
                            <li className="nav-item dropdown text-white">
                        ...
                        <div className="d-flex text-white">
                            ...
                            <button onClick={handleSearchSubmit} className="btn btn-outline-success_cus rounded me-2 text-white" type="submit">Search</button>
                        </div>
                        {isLoggedIn()
                            ?
                            <>
                                <Link className="link-no-underline btn-custom-primary rounded me-2" to={'/customer/account/'}>Account</Link>
                                <Link className="link-no-underline btn-custom-primary rounded me-2" to="/logout">Logout</Link>
                            </>
                            :
                            <>
                                <Link className="link-no-underline btn-custom-primary rounded me-2" to="/login">Login</Link>
                                <Link className="link-no-underline btn-custom-primary rounded me-2" to="/register">Register</Link>
